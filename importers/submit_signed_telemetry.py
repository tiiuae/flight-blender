...
class BlenderUploader:
    def upload_to_server(self, filename):
        ...
        eu_classification = rid_definitions.UAClassificationEU(
            category="Open", class_="Class0"
        )
        ...
        rid_operator_details = rid_definitions.RIDOperatorDetails(
            ...
            operator_location=rid_definitions.LatLngPoint(
                lat=46.97615311620088, lng=7.476099729537965
            ),
        )
        ...
        for state in states:
            ...
            try:
                ...
                signed_r = requests.Request(
                    method="PUT",
                    url="http://localhost:8000/flight_stream/set_signed_telemetry",
                    json=payload,
                    headers=headers,
                )
                ...
                signed_r.headers["Content-Digest"] = str(
                    http_sfv.Dictionary(
                        {"sha-256": hashlib.sha256(signed_r.body).digest()}
                    )
                )
