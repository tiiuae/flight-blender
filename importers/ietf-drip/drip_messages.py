...
MAX_AUTH_LENGTH = DRIP_AUTH_PAGE_ZERO_DATA_SIZE + DRIP_AUTH_PAGE_NONZERO_DATA_SIZE * (
    DRIP_AUTH_MAX_PAGES - 1
)
...
class DRIP_idtype_t(ctypes.c_int):
    ...
    DRIP_IDTYPE_UTM_ASSIGNED_UUID = (
        3,
    )  # UAS (Unmanned Aircraft System) Traffic Management
    DRIP_IDTYPE_SPECIFIC_SESSION_ID = (
        4,
...
class DRIP_SelfID_data(Structure):
    _fields_ = [
        ...
        (
            "Desc",
            c_char * (DRIP_STR_SIZE + 1),
        ),  # Additional byte to allow for null term in normative form
    ]
