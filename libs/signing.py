...
class MessageVerifier:
    def _get_public_keys(self):
        ...
        for current_public_key in all_public_keys:
            ...
            if redis.exists(redis_jwks_key):
                k = redis.get(redis_jwks_key)
                key = json.loads(k)
            else:
                response = session.get(current_public_key.url)
                ...
                redis.set(redis_jwks_key, json.dumps(key))
                redis.expire(redis_jwks_key, 60000)
            public_keys[current_kid] = key
        return public_keys

    def verify_message(self, request: HttpRequest) -> bool:
        stored_public_keys = self._get_public_keys()
        ...
        for _, jwk in stored_public_keys.items():
            verifier = HTTPMessageVerifier(
                signature_algorithm=algorithms.RSA_PSS_SHA512,
                key_resolver=BlenderHTTPSignatureKeyResolver(jwk=jwk),
            )
