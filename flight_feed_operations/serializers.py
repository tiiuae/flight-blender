...
class PositionSerializer(serializers.Serializer):
    ...
    accuracy_h = serializers.ChoiceField(
        choices=[(choice.value, choice.name) for choice in rid_dd.HorizontalAccuracy]
    )
    accuracy_v = serializers.ChoiceField(
        choices=[(choice.value, choice.name) for choice in rid_dd.VerticalAccuracy]
    )
...
class CurrentStateSerializer(serializers.Serializer):
    ...
    operational_status = serializers.ChoiceField(
        choices=[(choice.value, choice.name) for choice in rid_dd.RIDOperationalStatus]
    )
    position = PositionSerializer()
    track = serializers.FloatField()
    speed = serializers.FloatField()
    speed_accuracy = serializers.ChoiceField(
        choices=[(choice.value, choice.name) for choice in rid_dd.SpeedAccuracy]
    )
    ...
    group_time_start = serializers.DateTimeField(
        required=False, format="%Y-%m-%dT%H:%M:%S.%fZ"
    )
    group_time_end = serializers.DateTimeField(
        required=False, format="%Y-%m-%dT%H:%M:%S.%fZ"
    )
