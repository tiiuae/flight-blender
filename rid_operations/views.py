...
class RIDOutputHelper:
    def make_json_compatible(self, struct: Any) -> Any:
        if isinstance(struct, tuple) and hasattr(struct, "_asdict"):
            return {
                k: self.make_json_compatible(v) for k, v in struct._asdict().items()
            }
...
class SubscriptionHelper:
    ...
    def create_new_subscription(self, request_id, view: str, vertex_list: list):
        ...
        subscription_response = self.my_rid_output_helper.make_json_compatible(
            subscription_r
        )
...
def create_dss_subscription(request, *args, **kwargs):
    ...
    try:
        ...
        incorrect_parameters = {
            "message": "A view bounding box is necessary with four values: lat1,lng1,lat2,lng2."
        }
    ...
    subscription_r = my_subscription_helper.create_new_subscription(
...
def dss_isa_callback(request, subscription_id):
    ...
    try:
        ...
        for new_flight in service_areas:
            all_flights_url += (
                new_flight["flights_url"] + "?view=" + subscription_view + " "
            )

        flights_dict["all_uss_flights"] = all_flights_url
        r.hmset(flights_key, flights_dict)
        r.expire(
            name=flights_key, time=30
        )  # if a AOI is updated then keep the subscription active for 30 seconds
...
def get_flight_data(request, flight_id):
    ...
    if r.exists(flight_details_storage):
        ...
        return JsonResponse(
            json.loads(json.dumps(asdict(fd))), status=404, mimetype="application/json"
        )
...
def get_display_data(request):
    ...
    if view_port_valid:
        ...
        return JsonResponse(
            json.dumps(view_port_error), status=400, content_type="application/json"
        )
...
def create_test(request, test_id):
    ...
    try:
        ...
        msg = HTTPErrorResponse(
            message="Requested Flights not present in the payload", status=400
        )
        msg_dict = asdict(msg)
    ...
    if r.exists(test_id):
        ...
        stream_rid_test_data.delay(
            requested_flights=json.dumps(requested_flights)
    ...
    create_test_response = CreateTestResponse(
        injected_flights=requested_flights, version=1
    )
