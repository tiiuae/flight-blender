...
from .utils import GeoZoneParser
...
def download_geozone_source(geo_zone_url: str, geozone_source_id: str):
    r = get_redis()
    geo_awareness_test_data_store = "geoawarenes_test." + str(geozone_source_id)
    try:
    ...
    if r.exists(geo_awareness_test_data_store):
        r.set(geo_awareness_test_data_store, json.dumps(asdict(test_status_storage)))
...
def write_geo_zone(geo_zone: str, test_harness_datasource: str = "0"):
    ...
    ) = geo_zone_parser.parse_validate_geozone()
