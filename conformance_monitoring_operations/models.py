...
class TaskScheduler(models.Model):
    ...
    flight_declaration = models.OneToOneField(
        FlightDeclaration, on_delete=models.CASCADE
    )

    @staticmethod
    def schedule_every(
        task_name, period, every, flight_declaration, expires, args=None, kwargs=None
    ):
        ...
        if (
            ...
            interval_schedule.every = (
                every  # should check to make sure this is a positive int
            )
        ...
        return TaskScheduler.objects.create(
            periodic_task=ptask, flight_declaration=flight_declaration
        )
