...
def set_flight_declaration(request):
    print("Received flight application request ...")
    try:
    ...
    try:
        assert req.keys() >= {'originating_party', 'start_datetime',
                              'end_datetime', 'flight_declaration_geo_json', 'type_of_operation'}
    ...
    originating_party = 'No Flight Information' if 'originating_party' not in req else req[
        'originating_party']
    now = arrow.now()
    try:
        start_datetime = now.isoformat() if 'start_datetime' not in req else arrow.get(
            req['start_datetime']).isoformat()
        end_datetime = now.isoformat() if 'end_datetime' not in req else arrow.get(
            req['end_datetime']).isoformat()
    ...
    flight_declaration = FlightDeclarationRequest(features=all_features, type_of_operation=type_of_operation,
                                                  submitted_by=submitted_by, approved_by=approved_by, is_approved=is_approved, state=default_state)
    ...
    parital_op_int_ref = my_operational_intent_converter.create_partial_operational_intent_ref(
        geo_json_fc=flight_declaration_geo_json, start_datetime=start_datetime, end_datetime=end_datetime, priority=0)
    ...
    fence_within_timelimits = GeoFence.objects.filter(
        start_datetime__lte=start_datetime, end_datetime__gte=end_datetime).exists()
    all_relevant_fences = []
    if fence_within_timelimits:
        ...
        logging.info("Geofence intersections checked, found {num_intersections} fences" % {
    ...
    if existing_declaration_within_timelimits:
        ...
        logging.info("Flight Declaration intersections checked, found {num_intersections} declarations" % {
                     "all_relevant_declarations": len(relevant_id_set)})
        if all_relevant_declarations:
            is_approved = 0

    fo = FlightDeclaration(operational_intent=json.dumps(asdict(parital_op_int_ref)), bounds=bounds, type_of_operation=type_of_operation, submitted_by=submitted_by, is_approved=is_approved,
                           start_datetime=start_datetime, end_datetime=end_datetime, originating_party=originating_party, flight_declaration_raw_geojson=json.dumps(flight_declaration_geo_json), state=default_state)
    fo.save()

    flight_declaration_id = str(fo.id)
    amqp_connection_url = env.get('AMQP_URL', 0)
    print(
        "Sending operational update message about flight declaration created if there is AMQP connection ...")
    if amqp_connection_url:
        print(
            "Sending operational update message about flight declaration created ...")
        send_operational_update_message.delay(
            flight_declaration_id=flight_declaration_id, message_text="Flight Declaration created..", level='info')

    print("Checking self deconfliction, if there is any, wont be able to send to DSS...")
    if all_relevant_fences and all_relevant_declarations:
        # Async submic flight declaration to DSS
        print("Self deconfliction failed")
        logger.info(
        ...
        if amqp_connection_url:
            ...
            send_operational_update_message.delay(
                flight_declaration_id=flight_declaration_id, message_text=self_deconfliction_failed_msg, level='error')

    else:
        print("No self deconfliction, sending to dss ...")
        logger.info(
    ...
    creation_response = FlightDeclarationCreateResponse(
        id=flight_declaration_id, message="Submitted Flight Declaration", is_approved=is_approved, state=default_state)
...
class FlightDeclarationList(mixins.ListModelMixin,
    ...
    def get_relevant_flight_declaration(self, start_date, end_date,  view_port: List[float]):
        ...
        logging.info("Found %s flight declarations" %
                     len(all_fd_within_timelimits))
        if view_port:
