...
def SCDClearAreaRequest(request):
        ...
        for flight_details in all_existing_op_ints_in_area:
            ...
            if op_int_details:
                ...
                my_scd_dss_helper.delete_operational_intent(
                    operational_intent_id=opint_id, ovn=ovn
                )
        clear_area_status = ClearAreaResponse(
            success=True,
    ...
    return JsonResponse(
        json.loads(json.dumps(clear_area_status, cls=EnhancedJSONEncoder)), status=200
    )
    ...
    if request.method == "PUT":
        ...
        try:
            ...
            for volume in operational_intent_volumes:
                ...
                time_end = Time(
                    format=volume["time_end"]["format"],
                    value=volume["time_end"]["value"],
                )
        ...
        if "error" in auth_token:
            ...
            op_int_submission = my_scd_dss_helper.create_and_submit_operational_intent_reference(
                state=test_injection_data.operational_intent_data.state,
                volumes=test_injection_data.operational_intent_data.volumes,
                off_nominal_volumes=test_injection_data.operational_intent_data.off_nominal_volumes,
                priority=test_injection_data.operational_intent_data.priority,
            )
            if op_int_submission.status == "success":
                ...
                operational_intent_full_details = OperationalIntentStorage(
                    ...
                    operational_intent_details=test_injection_data.operational_intent_data,
                )
                # Store flight ID
                ...
                flight_op_int_storage = SuccessfulOperationalIntentFlightIDStorage(
                    flight_id=str(operation_id),
                    operational_intent_id=operational_intent_data.off_nominal_volumes,
                )
