...
class SCDOperations:
    ...
    def delete_operational_intent(self, operational_intent_id: uuid.uuid4, ovn: str):
        ...
        delete_payload = DeleteOperationalIntentConstuctor(
            entity_id=operational_intent_id, ovn=ovn
        )
            ...
            for (
                ...
                try:
                    ...
                    if uss_operational_intent_request.status_code == 200:
                        ...
                        time_start = Time(
                            format=op_int_ref["time_start"]["format"],
                            value=op_int_ref["time_start"]["value"],
                        )

                        time_end = Time(
                            format=op_int_ref["time_end"]["format"],
                            value=op_int_ref["time_end"]["value"],
                        )

                        op_int_reference = OperationalIntentReferenceDSSResponse(
                            id=op_int_ref["id"],
                            uss_availability=op_int_ref["uss_availability"],
                            manager=op_int_ref["manager"],
                            version=op_int_ref["version"],
                            state=op_int_ref["state"],
                            ovn=op_int_ref["ovn"],
                            time_start=time_start,
                            time_end=time_end,
                            uss_base_url=op_int_ref["uss_base_url"],
                            subscription_id=op_int_ref["subscription_id"],
                        )
    ...
    def create_and_submit_operational_intent_reference(
        ...
        if deconflicted:
            ...
            if dss_r_status_code in [200, 201]:
                ...
                time_start = Time(
                    format=o_i_r["time_start"]["format"],
                    value=o_i_r["time_start"]["value"],
                )
                time_end = Time(
                    format=o_i_r["time_end"]["format"], value=o_i_r["time_end"]["value"]
                )
